...
Usage:
  learn_model_proba_speaking_face.py <videoList> <rawfaceTrackPositionPath> <descFaceSelection> <facePositionReference> <modelFaceSelection_out>
...
if __name__ == '__main__':
    ...
    for videoID in open(args['<videoList>']).read().splitlines():
        ...
        ref_f = read_ref_facetrack_position(args['<facePositionReference>'], videoID, 3)
        facetracks = {}
        l_facetrack_used_to_learn_model = set([])
        for line in open(args['<rawfaceTrackPositionPath>']+'/'+videoID+'.facetrack').read().splitlines():
    ...
    clf.fit(X, Y)
    # save model     
    joblib.dump(clf, args['<modelFaceSelection_out>']) 
